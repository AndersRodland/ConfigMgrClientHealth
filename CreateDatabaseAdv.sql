...
GO
begin tran
if exists (SELECT * FROM dbo.{Configuration} WITH (updlock,serializable) WHERE Name='{DatabaseName}')
begin
    IF EXISTS (SELECT * FROM dbo.{Configuration} WITH (updlock,serializable) WHERE Name='{DatabaseName}' AND Version < '0.7.5')
    UPDATE dbo.{Configuration} SET Version='0.7.5' WHERE Name = '{DatabaseName}'
end
else
begin
    INSERT INTO dbo.{Configuration} (Name, Version)
    VALUES ('{DatabaseName}', '0.7.5')
end
commit tran
